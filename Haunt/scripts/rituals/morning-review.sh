...
# Enhanced daily morning review script for Haunt
...
echo ""
echo -e "${BOLD}${BLUE}╔═══════════════════════════════════════════════════════════╗${NC}"
echo -e "${BOLD}${BLUE}║                 MORNING SÉANCE                            ║${NC}"
echo -e "${BOLD}${BLUE}║         Ghost County Haunt - $(date +"%Y-%m-%d %H:%M")         ║${NC}"
echo -e "${BOLD}${BLUE}╚═══════════════════════════════════════════════════════════╝${NC}"
echo ""
...
if [[ -f ".haunt/plans/roadmap.md" ]]; then
    ...
    print_error "No haunting manifest found at .haunt/plans/roadmap.md"
    health_max=$((health_max + 15))
fi
...
if [[ -f ".haunt/plans/roadmap.md" ]]; then
    in_progress_items=$(grep -E "🟡.*REQ-[0-9]+" .haunt/plans/roadmap.md 2>/dev/null)

    if [[ -n "$in_progress_items" ]]; then
        echo ""
        echo -e "${YELLOW}${BOLD}Active Hauntings:${NC}"
        echo "$in_progress_items" | head -5
    else
        print_info "No active hauntings in progress"
    fi

    blocked_items=$(grep -E "🔴.*REQ-[0-9]+" .haunt/plans/roadmap.md 2>/dev/null)
...
if command -v pytest &> /dev/null; then
    if [[ -d "tests" || -d ".haunt/tests" ]]; then
        test_output=$(pytest tests/ .haunt/tests/ -q --tb=no 2>&1 || true)
        test_exit=$?
...
echo ""
echo -e "${BOLD}${BLUE}═══════════════════════════════════════════════════════════${NC}"
echo -e "${GREEN}${BOLD}Morning séance complete!${NC} May your hauntings be productive! 👻"
echo -e "${BOLD}${BLUE}═══════════════════════════════════════════════════════════${NC}"
echo ""
