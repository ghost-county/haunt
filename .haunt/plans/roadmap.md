...
**Tasks:**
- [x] Update `gco-ui-testing.md` with "NON-NEGOTIABLE" language
- [x] Add explicit failure cases: "If E2E tests don't exist, requirement CANNOT be 🟢"
- [x] Update `gco-completion-checklist.md` step 3 to include E2E test requirement
- [x] Add verification script: check for E2E test files before allowing 🟢
- [x] Document consequences of bypassing (mark as incomplete, flag for review)

**Files:**
- `.claude/rules/gco-ui-testing.md` (created)
- `.claude/rules/gco-completion-checklist.md` (updated step 3)
- `Haunt/rules/gco-ui-testing.md` (created - source)
- `Haunt/rules/gco-completion-checklist.md` (updated - source)
- `Haunt/scripts/verify-e2e-tests.sh` (created - executable)

**Effort:** S
**Complexity:** SIMPLE
**Agent:** Dev-Infrastructure
**Completion:** Rules updated with mandatory language. Verification script exists and tested. Documentation includes explicit "CANNOT mark complete without tests" statements and consequences section. All changes deployed to local and global .claude directories.
**Blocked by:** None
...
**Tasks:**
- [x] Create `.haunt/scripts/pre-commit-e2e-check.sh`
- [x] Detect UI file changes (components/, pages/, *.tsx, *.jsx)
- [x] Check for corresponding E2E test file in tests/e2e/
- [x] Run E2E tests if they exist (optional via RUN_E2E_TESTS env var)
- [x] Block commit if tests missing or failing for UI work
- [x] Integrate into setup script as optional enforcement

**Files:**
- `.haunt/scripts/pre-commit-e2e-check.sh` (created)
- `.git/hooks/pre-commit` (generated by setup script)
- `Haunt/scripts/setup-haunt.sh` (integrated)
...
**Implementation Notes:**
- Hook uses smart detection: blocks commits without E2E test files, but doesn't run tests by default (too slow for pre-commit)
- Environment variables for configuration:
  - `STRICT_E2E_MODE=false` to disable blocking (default: true)
  - `RUN_E2E_TESTS=true` to run tests on every commit (default: false for speed)
- Bypass available for emergencies: `git commit --no-verify`
- Hook installed automatically by `setup-haunt.sh` during project structure setup
- Tested successfully: detects UI files, checks for tests, blocks when missing, allows when present
