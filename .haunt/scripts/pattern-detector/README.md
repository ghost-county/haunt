...
python3 generate_tests.py --input patterns.json --output-dir .haunt/tests/patterns --mock

# Dry run (generate but don't write files)
python3 generate_tests.py --input patterns.json --mock --dry-run

# Validate existing tests
python3 generate_tests.py --output-dir .haunt/tests/patterns --validate-only
```
...
| `--output-dir` | Output directory | `.haunt/tests/patterns` |
...
# Write to file
generator.write_test_files([result], '.haunt/tests/patterns')
```
...
python3 generate_tests.py --input patterns.json --output-dir .haunt/tests/patterns
...
# Custom paths
python3 update_precommit.py --config /path/to/.pre-commit-config.yaml --test-dir .haunt/tests/patterns
...
| `--test-dir` | Path to test directory | `.haunt/tests/patterns` |
| `--dry-run` | Show changes without applying | False |
...
# Run updater
python3 update_precommit.py --test-dir .haunt/tests/patterns
...
# → pre-commit runs pytest .haunt/tests/patterns/ -v
```
...
2. Apply approved updates to agent character sheets in `Haunt/agents/`
