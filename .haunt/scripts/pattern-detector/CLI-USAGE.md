...
→ Results saved in: .haunt/pattern-hunter
```
...
**Output:** Python test files in `.haunt/tests/patterns/`:
```
...
The CLI maintains state between runs in `.haunt/pattern-hunter/state.json`:
...
All intermediate and final outputs are saved in `.haunt/pattern-hunter/`:

```
.haunt/pattern-hunter/
...
Generated tests are saved in `.haunt/tests/patterns/`:
...
5. **Test defeat tests** - Run `pytest .haunt/tests/patterns/` after generation
...
- [Ghost County Framework](../../README.md) - Overall framework
