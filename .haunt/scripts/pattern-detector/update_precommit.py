...
class PreCommitUpdater:
    ...
    def __init__(
        ...
        Args:
            ...
            test_dir: Path to test directory (default: .haunt/tests/patterns)
        """
        self.config_path = config_path or Path('.pre-commit-config.yaml')
        self.test_dir = test_dir or Path('.haunt/tests/patterns')
...
def main():
    ...
    parser.add_argument(
        '--test-dir',
        type=Path,
        help='Path to test directory (default: .haunt/tests/patterns)'
    )
    parser.add_argument(
        '--dry-run',
        action='store_true',
