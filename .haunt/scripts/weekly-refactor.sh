...
# Weekly refactor ritual script for Ghost County
...
WEEK_NUM=$(date +%U)
REPORT_FILE=".haunt/progress/weekly-refactor-week-${WEEK_NUM}-${WEEK_DATE}.md"
        ...
        if [ "$PATTERN_COUNT" -gt 0 ]; then
            ...
            if command -v pytest &> /dev/null; then
                ...
                if [ "$DRY_RUN" = false ]; then
                    if pytest .haunt/tests/patterns/ -v --tb=short 2>/dev/null; then
                        print_success "All pattern tests passed"
                    else
                        print_warning "Some pattern tests failed (new patterns detected?)"
                    fi
                else
                    echo -e "${YELLOW}[DRY RUN]${NC} Would run: pytest .haunt/tests/patterns/ -v --tb=short"
                fi
            else
...
EOF
        ...
        if [ -f ".haunt/plans/roadmap.md" ]; then
            COMPLETED_COUNT=$(grep -c "🟢" .haunt/plans/roadmap.md 2>/dev/null || echo "0")
            IN_PROGRESS_COUNT=$(grep -c "🟡" .haunt/plans/roadmap.md 2>/dev/null || echo "0")
            BLOCKED_COUNT=$(grep -c "🔴" .haunt/plans/roadmap.md 2>/dev/null || echo "0")
            NOT_STARTED_COUNT=$(grep -c "⚪" .haunt/plans/roadmap.md 2>/dev/null || echo "0")
...
EOF
        ...
        echo ""
        if [ -f ".haunt/plans/roadmap.md" ]; then
            echo "  Completed: $(grep -c "🟢" .haunt/plans/roadmap.md 2>/dev/null || echo "0")"
            echo "  In Progress: $(grep -c "🟡" .haunt/plans/roadmap.md 2>/dev/null || echo "0")"
            echo "  Blocked: $(grep -c "🔴" .haunt/plans/roadmap.md 2>/dev/null || echo "0")"
        fi
        echo "  Commits (7 days): $(git log --since="7 days ago" --oneline 2>/dev/null | wc -l | tr -d ' ')"
        echo "  Pattern tests: $(find .haunt/tests/patterns -name "test_*.py" 2>/dev/null | wc -l | tr -d ' ')"
...
if [ "$SKIP_ARCHIVE" = false ]; then
    ...
    if [ -f ".haunt/plans/roadmap.md" ]; then
        ...
        if [ "$COMPLETED_COUNT" -gt 0 ]; then
            ...
            ARCHIVE_FILE=".haunt/completed/week-${WEEK_NUM}-${ARCHIVE_DATE}.md"
...
EOF
                ...
                grep -B 5 "🟢" .haunt/plans/roadmap.md 2>/dev/null | head -50 >> "$ARCHIVE_FILE" || true

                print_success "Created archive: $ARCHIVE_FILE"
                print_warning "Note: Review archive and manually clean roadmap.md"
                print_info "Remove completed items from .haunt/plans/roadmap.md after verification"
            else
        ...
        print_warning "Roadmap file not found: .haunt/plans/roadmap.md"
